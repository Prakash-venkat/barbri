...
class SubMenu extends React.Component {
  ...
  render() {
    return (
      <div>
        <NavItem onClick={this.toggleNavbar} className={classNames({ 'menu-open': !this.state.collapsed })} style={{ padding: !this.state.collapsed ? '1% 1%' : '0% 1%' }}>
            ...
            <NavLink className="dropdown-toggle" activeClassName="active" exact={true}>
              <Col xs='12' sm='12' md='12' lg='12' xl='12' className="p-0">
                <Row className="m-0">
                  ...
                  <Col xs='2' sm='2' md='2' lg='2' xl='2' className="p-0 text-right">
                    {!this.state.collapsed ? <i class="pe-7s-angle-down"></i> :
                      <i class="pe-7s-angle-right"></i>}
                  </Col>
            ...
            <NavItem key={item.key} className="pl-4">
              <NavLink tag={RouterLink} activeStyle={{ color: '#0e6aae', textDecoration: 'none', boxShadow: 'inset -4px 0 0 #0e6aae', background: '#eee' }} to={item.target} activeClassName="active" exact={true}>
                {item.submenu ?
                  ...
                  <div>
                    <NavItem style={{ padding: '0%' }} onClick={() => this.subToggleNavbar(item.key)} className={classNames({ 'menu-open': !this.state.subCollapsed })} style={{ padding: !this.state.collapsed ? '1% 1%' : '4% 1%' }}>
                      <NavLink className="dropdown-toggle" style={{ padding: '0%' }} activeClassName="active" exact={true}>
                        <Col xs='12' sm='12' md='12' lg='12' xl='12' className="p-0">
                          <Row className="m-0">
                            <Col xs='10' sm='10' md='10' lg='10' xl='10' className="p-0">
                            ...
                            <Col xs='2' sm='2' md='2' lg='2' xl='2' className="p-0 text-right">
                              {!this.state.subCollapsed ? <i class="pe-7s-angle-down"></i> :
                                <i class="pe-7s-angle-right"></i>}
                            </Col>
                    ...
                    {this.state.IndexValue === item.key ?
                      <Collapse isOpen={!this.state.subCollapsed} navbar className={classNames('items-menu', { 'mb-1': !this.state.subCollapsed })}>
                        {item.submenu.map((submenu, index) => (
                          <NavItem className="pl-4" key={submenu.key}>
                            <NavLink tag={RouterLink} activeStyle={{ color: '#0e6aae', textDecoration: 'none', boxShadow: 'inset -4px 0 0 #0e6aae', background: '#eee' }} to={submenu.target} activeClassName="active" exact={true}>
                              {submenu.title}
                            </NavLink>
                          </NavItem>
                        ))}
                      </Collapse> : console.log("No Submenu in sidebar")
                    }
